...
public class ModuleApiUtil {
    ...
    public List<DeliveryAddressDto> getUserDeliveryAddressApi(Long userId) {
        Map<String, String> headers = Map.of(
                HttpHeaders.CONTENT_TYPE, "application/json",
                HttpHeaders.AUTHORIZATION, "Bearer " + userId);
        return webClientUtil.get(userApi + "/DeliveryAddressApi", new ParameterizedTypeReference<List<DeliveryAddressDto>>() {
        }, null, headers);
    }
    ...
    public List<ItemDetailDto> getItemDetailFromCartApi(Long userId) {
        Map<String, String> headers = Map.of(
                HttpHeaders.CONTENT_TYPE, "application/json",
                HttpHeaders.AUTHORIZATION, "Bearer " + userId);
        return webClientUtil.get(cartApi + "/ItemDetailApi", new ParameterizedTypeReference<List<ItemDetailDto>>() {
        }, null, headers);
    }
