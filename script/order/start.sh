...
echo "> $JAR_NAME 를 profile=order-$IDLE_PROFILE 로 실행합니다."

cd $REPOSITORY

sudo docker build -t order -f $REPOSITORY/zip/Dockerfile . || exit 1
sudo docker run -it --name "order-$IDLE_PROFILE" -d --env-file $REPOSITORY/zip/.env -e SPRING_PROFILES_ACTIVE=$IDLE_PROFILE -p $IDLE_PORT:$IDLE_PORT order || exit 1