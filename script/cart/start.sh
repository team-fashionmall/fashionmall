...
sudo docker run -it --name "coupon-$IDLE_PROFILE" -d --env-file $REPOSITORY/zip/.env -e SPRING_PROFILES_ACTIVE=$IDLE_PROFILE -p $IDLE_PORT:$IDLE_PORT coupon || exit 1 #컨테이너 실행(모듈별)